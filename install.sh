...
# Lee cada línea del fichero env en un array, preservando espacios en los valores
mapfile -t ENV_ARGS < <(grep -v '^[[:space:]]*#' "$CONFIG_DIR/env" | grep -v '^[[:space:]]*$')
BUILD_LOG=$(sudo -u "$APP_USER" env "${ENV_ARGS[@]}" bash -c "cd $APP_DIR && npm run build 2>&1")
BUILD_EXIT=$?
...
sudo -u "$APP_USER" env "${ENV_ARGS[@]}" bash -c "cd $APP_DIR && npx drizzle-kit push --force 2>&1" | tail -5
