...
-   **Technical Implementations & Feature Specifications**:
    ...
    -   **LOPD Compliance**: `audit_logs` table records security-relevant actions (login/logout, student CRUD, user management, permission changes, settings changes, archive/reset, cleanup). Admin-only audit page at `/audit` with filters by action/entity and pagination. Manual data cleanup (1-10 years) via `/api/admin/cleanup`. Automatic daily cleanup of records older than 3 years (exit logs, late arrivals, chat, DM, notifications, incidents). Privacy policy page at `/privacy` accessible without authentication, linked from login page.
