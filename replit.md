...
## Data Models
- `users` - Admin and guard/teacher accounts with roles, optional email for password recovery, `permissions` text[] for granular admin panel access
...
- **Granular Permission System**: Admin can assign specific admin panel permissions to guard/tutor users via the Profesores page. 15 permission keys (students, groups, teachers, calendar, late_arrivals, history, late_history, print, scan, guard_duty, guard_registry, absences, schedules, archives, settings). Users with permissions see the admin panel with sidebar filtered to permitted sections. Frontend enforces per-route `PermissionGate` checks. Backend enforces via `requirePermission()` middleware. Non-admin users with permissions see a "Vista de profesor" button in sidebar footer to switch back to staff view.
...
- **guard**: StaffView with bottom tab navigation — 4 tabs: "Guardia" (QR verification) + "Tardías" (late arrivals) + "Fichar" (guard duty sign-in) + "Ausencias" (register/view own absences). No sidebar, mobile/tablet optimized. If the guard has admin permissions assigned, they see the admin panel instead (with sidebar filtered to permitted sections and a "Vista de profesor" button to switch to staff view).
- **tutor**: StaffView with bottom tab navigation — 6 tabs: "Mi Grupo" (student management, photos, carnet sharing) + "Guardia" (QR verification) + "Tardías" (late arrivals) + "Fichar" (guard duty sign-in) + "Ausencias" (register/view own absences) + "Registros" (exit logs and late arrivals history for their group). No sidebar, mobile optimized. If the tutor has admin permissions assigned, they see the admin panel instead (with sidebar filtered and staff view toggle).
